...
import type { AdminBrandDetail, BrandStatus } from '../../../interfaces/admin-brand.interface';
...
@Component({
  ...
  styleUrl: '../brand-create/brand-create.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class BrandEditPage {
  ...
  private readonly brandId = this.route.snapshot.paramMap.get('term') ?? '';
  ...
  readonly brand = signal<AdminBrandDetail | null>(null);
  ...
  readonly selectedImageName = computed(
    () => (this.selectedImage()?.name ?? this.currentImageName()) || 'Sin imagen cargada',
  );
  readonly previewImageUrl = computed(
    () => this.previewUrl() || this.currentImageUrl() || BRAND_IMAGE_PLACEHOLDER,
  );
  ...
  submit(): void {
    ...
    this.brandsService
      ...
      .subscribe({
        next: (brand) => {
          this.toastService.success('Marca actualizada', `La marca "${brand.name}" fue actualizada correctamente.`);
          this.router.navigate(['/admin/marcas', brand.id], {
            queryParamsHandling: 'preserve',
          });
        },
        error: (err: { error?: { message?: string }; message?: string }) => {
          this.submitError.set(err?.error?.message ?? err?.message ?? 'No se pudo actualizar la marca');
        },
      });
  }
  ...
  private loadBrand(): void {
    if (!this.brandId) {
      ...
      this.loadError.set('Marca no especificada');
      return;
    }
      ...
      .subscribe({
        next: (brand) => {
          ...
          this.form.patchValue({
            ...
            backgroundColor: brand.backgroundColor || DEFAULT_BACKGROUND,
          });
          this.currentImageUrl.set(brand.imageUrl || brand.image || BRAND_IMAGE_PLACEHOLDER);
          this.currentImageName.set(brand.imageKey || 'Imagen actual');
          this.loading.set(false);
        },
        error: (err: { error?: { message?: string }; message?: string }) => {
          this.loadError.set(err?.error?.message ?? err?.message ?? 'No se pudo cargar la marca');
          this.loading.set(false);
        },
      });
  }
